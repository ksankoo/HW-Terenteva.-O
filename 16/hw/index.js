...
const checkValue = function (value, regexp) {
    ...
    inputPassword.addEventListener('input', function () {
        ...
        if (validate === false) {
            document.querySelector("#password-error").innerHTML = "Длина пароля должна быть от 8 до 20 символов; пароль должен содержать хотя бы одну строчную букву, одну заглавную букву, одну цифру, один специальный символ из набора !@#$%^&*()";
    ...
    repeatPassword.addEventListener('input', function () {
        if (inputPassword.value !== repeatPassword.value) {
            document.querySelector("#password-repeat-error").textContent = "Неверно введен пароль";
...
function makeSubmit(event) {
    ...
    if (isNameValid && isEmailValid && isAgeValid && sexInputCheck() && occupationCheck() && isPasswordValid && isPasswordSame && checkboxCheck()) {
        ...
        document.querySelector("#submit-error").textContent = "Проверьте правильность введенных данных";
        ...
        if (occupationCheck() === false) {
            document.querySelector("#occupation-error").innerHTML = "Выберите профессию";
        ...
        if (checkboxCheck() === false) {
            document.querySelector("#checkbox-error").textContent = "Необходимо согласие на обработку персональных данных";
            checkbox.classList.add('error');
        }
    }
}
// возраст пол профессия обработка
